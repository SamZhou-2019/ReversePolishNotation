...
def reverse(str_retrieve):
    ...
    _keywords_ = str_retrieve.replace('(', '')
    _keywords_ = _keywords_.replace('（', '')
    _keywords_ = _keywords_.replace(')', '')
    _keywords_ = _keywords_.replace('）', '')
    ...
    for str_no in range(0, len(str_retrieve)):
        # 左括号：直接入栈
        if str_retrieve[str_no] == '(' or str_retrieve[str_no] == '（':
            isChar = True
            _char_.append('(')
        # +-*：需判断当前扫描的算符与算符栈栈顶算符的优先级
        elif str_retrieve[str_no] == '+' or str_retrieve[str_no] == '*' or str_retrieve[str_no] == '-':
            isChar = True
        ...
        elif str_retrieve[str_no] == ')' or str_retrieve[str_no] == '）':
            isChar = True
            while _char_[-1] != '(':
                _result_.append(_char_.pop())
            _char_.pop()
